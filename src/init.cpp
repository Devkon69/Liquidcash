 ...
 * between liquidcashd, and liquidcash-qt and non-server tools.
*/
...
void Shutdown()
{
    ...
    RenameThread("liquidcash-shutoff");
    ...
    FlushWallets();
#endif
    GenerateLiquidcashs(false, 0, Params());
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), LCASH_PID_FILENAME));
#endif
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/liquidcashproject/Titan>";
...
void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    RenameThread("liquidcash-loadblk");
...
/** Sanity checks
 *  Ensure that Liquidcash is running in a usable environment with all
...
void InitLogging()
{
    ...
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Liquidcash version %s\n", FormatFullVersion());
}
...
static bool LockDataDirectory(bool probeOnly)
{
    ...
    // Make sure only a single Liquidcash process is using the data directory.
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", LCASH_CONF_FILENAME)).string());
    ...
    GenerateLiquidcashs(fGenerate, gArgs.GetArg("-genproclimit", DEFAULT_GENERATE_THREADS), chainparams);
