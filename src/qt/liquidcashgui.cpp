...
const std::string LiquidcashGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString LiquidcashGUI::DEFAULT_WALLET = "~Default";

LiquidcashGUI::LiquidcashGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
    clientModel(0),
    walletFrame(0),
...
void LiquidcashGUI::createActions()
{
    QFont font = QFont();
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Liquidcash address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and liquidcash: URIs)"));
    ...
    transferAssetAction->setStatusTip(tr("Transfer assets to LCASH addresses"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Liquidcash addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Liquidcash addresses"));
    ...
    openAction->setStatusTip(tr("Open a liquidcash: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Liquidcash command-line options").arg(tr(PACKAGE_NAME)));
...
void LiquidcashGUI::createMenuBar()
{
#ifdef Q_OS_MAC
...
void LiquidcashGUI::createToolBars()
{
    if(walletFrame)
    {
        /** LCASH START */
        ...
        label->setPixmap(QPixmap::fromImage(QImage(":/icons/liquidcashtext")));
...
//        toolbar->addAction(votingAction);
        ...
        QVBoxLayout* liquidcashLabelLayout = new QVBoxLayout(toolbarWidget);
        liquidcashLabelLayout->addWidget(label);
        liquidcashLabelLayout->addWidget(toolbar);
        liquidcashLabelLayout->setDirection(QBoxLayout::TopToBottom);
        liquidcashLabelLayout->addStretch(1);
...
void LiquidcashGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool LiquidcashGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool LiquidcashGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void LiquidcashGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void LiquidcashGUI::setWalletActionsEnabled(bool enabled)
{
...
void LiquidcashGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void LiquidcashGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void LiquidcashGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void LiquidcashGUI::optionsClicked()
{
...
void LiquidcashGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void LiquidcashGUI::showDebugWindow()
{
...
void LiquidcashGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void LiquidcashGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void LiquidcashGUI::openClicked()
{
...
void LiquidcashGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void LiquidcashGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void LiquidcashGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void LiquidcashGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void LiquidcashGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void LiquidcashGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}

/** LCASH START */
void LiquidcashGUI::gotoAssetsPage()
{
    transferAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoAssetsPage();
};

void LiquidcashGUI::gotoCreateAssetsPage()
{
    createAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoCreateAssetsPage();
};

void LiquidcashGUI::gotoManageAssetsPage()
{
...
void LiquidcashGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Liquidcash network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void LiquidcashGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void LiquidcashGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void LiquidcashGUI::updateHeadersSyncProgressLabel()
{
...
void LiquidcashGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void LiquidcashGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Liquidcash"); // default title
...
void LiquidcashGUI::changeEvent(QEvent *e)
{
...
void LiquidcashGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void LiquidcashGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void LiquidcashGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& assetName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date);
    if (assetName == "LCASH")
        msg += tr("Amount: %1\n").arg(LiquidcashUnits::formatWithUnit(unit, amount, true));
    else
        msg += tr("Amount: %1\n").arg(LiquidcashUnits::formatWithCustomName(assetName, amount, MAX_ASSET_UNITS, true));
...
void LiquidcashGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void LiquidcashGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool LiquidcashGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool LiquidcashGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void LiquidcashGUI::setHDStatus(int hdEnabled)
{
...
void LiquidcashGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void LiquidcashGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void LiquidcashGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void LiquidcashGUI::detectShutdown()
{
...
void LiquidcashGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void LiquidcashGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void LiquidcashGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(LiquidcashGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void LiquidcashGUI::subscribeToCoreSignals()
{
...
void LiquidcashGUI::unsubscribeFromCoreSignals()
{
...
void LiquidcashGUI::toggleNetworkActive()
{
    if (clientModel) {
    ...
    menu(0)
{
    ...
    QList<LiquidcashUnits::Unit> units = LiquidcashUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const LiquidcashUnits::Unit unit : units)
    {
        max_width = qMax(max_width, fm.width(LiquidcashUnits::name(unit)));
    }
...
{
    menu = new QMenu(this);
    for (LiquidcashUnits::Unit u : LiquidcashUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(LiquidcashUnits::name(u)), this);
...
{
    setText(LiquidcashUnits::name(newUnits));
}
...
void LiquidcashGUI::getPriceInfo()
{
    request->setUrl(QUrl("https://api.binance.com/api/v1/ticker/price?symbol=LCASHBTC"));
