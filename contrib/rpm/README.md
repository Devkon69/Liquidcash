...
The RPM spec file provided here is for Liquidcash-Core 0.12.0 and builds on CentOS
...
[Liquidcash git repository](https://github.com/liquidcashproject/Titan) but are not part of
...
Currently only `Source10` is used, for the example `liquidcash.conf` file.
...
in the liquidcash git repository. At present only an SVG version of the Liquidcash
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the liquidcash
...
needed when 0.12.1 is released, a proper fix is already in the Liquidcash git
master branch.
...
idea to build Liquidcash Core against a static version of that library compiled
...
The `liquidcash.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `liquidcash-server` package. At this point, most RPM based
...
The `liquidcash-server` package should have SELinux support. How to properly do
...
Liquidcash-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/liquidcash/). He has been
packaging Liquidcash for Fedora at least since 2012.
...
4. I renamed the `liquidcash` package that contains the Qt GUI to `liquidcash-core` as
that appears to be how the general population refers to it, in contrast to
`liquidcash-xt` or `liquidcash-classic`. I wanted to make sure the general population
